...
export default function Movies({ data }: { data: { results: IMovie[] } }) {
  ...
  useEffect(() => {
    ...
    setSortedMovies(filteredAndSortedMovies);
  }, [optionalFilter, searchContent, ...sortedMovies]);
            ...
            <Dropdown>
              <DropdownTrigger>
                <div className="flex bg-slate-950 text-white rounded-3xl px-4 py-2">
                  <div className="flex gap-2">
                    ...
                    <span className="text-xs capitalize">{optionalFilter}</span>
                  </div>
              ...
              >
                ...
                <DropdownItem
                  className="rounded-lg py-1"
                  key="release date"
                  startContent={<span className="lets-icons--date-fill"></span>}
                >
                  Release Date
                </DropdownItem>
              </DropdownMenu>
            ...
            {sortedMovies.map((movie, index) => (
              <Link key={index} href={`/movie/${movie.id}`}>
                <div
                  ...
                  } snap-start flex rounded-3xl border border-zinc-400/30 justify-start items-end`}
                  style={{
